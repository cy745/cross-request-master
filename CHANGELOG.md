...
- **沉浸式仍残留"备注"标题** - 4.5.20 收掉了容器空白，但 `备注` h2 标题文字本身还显示。沉浸态下视野里只有文档内容，没必要重复提示"这是备注"，本版本把 `h2.interface-title[data-crm-section="notes"]` 也隐掉，让 iframe 直接顶到 tab bar 下方。

## [4.5.20] - 2026-05-15

### 修复

- **沉浸式查看模式残留空白** - 4.5.19 隐藏了 `基本信息 / 请求参数 / 返回数据` 三块内容，但外层容器 `.caseContainer` 的 `padding-top: 24px` 和 "备注" h2 的默认 `margin-top: 48px` 没收掉，备注标题会被顶离 tab bar 约 72px。本版本在沉浸态下把 `.caseContainer` 上边距收到 12px、把所有 `.interface-title` 的 `margin-top` 归零，让备注紧贴 tab bar。

## [4.5.19] - 2026-05-15

### 新增

- **文档型接口沉浸式查看模式** - 当接口的 `desc` 是 `<iframe srcdoc>` 包装的 HTML 文档（由 `@leeguoo/yapi-mcp` docs-sync 0.6.1+ 推上来）或 `markdown` 字段以 docs-sync 警告横幅开头时，自动隐藏页面里的"基本信息 / 请求参数 / 返回数据"三个区块，让备注里的 iframe 文档独占视野；右上角浮动 **「↩ 退出沉浸式」** 按钮可手动切回完整视图，本会话内不再自动重入，刷新页面重置。

## [4.5.15] - 2026-01-06

### 新增

- **YApi 工具箱** - 接口详情页整合 MCP 配置 / Skill 安装 / CLI 与 docs-sync 说明

### 变更

- **复制给 AI** - 独立为快捷按钮，避免与配置能力混在同一面板

## [4.5.14] - 2026-01-02

### 新增

- **固定 Header** - 支持为当前站点配置并自动追加自定义 Header
...
- **路径参数弹窗** - 支持识别 URL 中的 `{param}` 并引导填写，避免请求失败
- **cURL 内嵌展示** - YApi「运行」页在 URL 行下方显示 cURL，不再遮挡页面
- **问题反馈入口** - cURL 面板新增「反馈」按钮直达 GitHub Issues

### 修复

- **运行页资源加载** - 修复新增 helper 未加入 `web_accessible_resources` 导致加载失败

### 变更

- **移除永久关闭** - 不再支持 cURL「永久关闭」按钮与相关存储逻辑
